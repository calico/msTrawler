...
The `mstrawler` CLI avoids the need to write Python scripts for common tasks.
...
mstrawler run \
  ...
  --covariate covariate_file.csv \
  --output ./results \
  --backend python \
  --seed 42
```

### Convert files only (no modeling)

```bash
mstrawler convert --psm PSMs.txt --protein Proteins.txt
```
...
```bash
mstrawler check
# [OK] python: statsmodels WLS + Wald F-tests (always available)
# [NOT AVAILABLE] r: rpy2 is required for the 'r' backend...
```

### All CLI options for `mstrawler run`

| Flag | Default | Description |
|------|---------|-------------|
| `--psm` | (required) | Path to Proteome Discoverer PSM file |
| `--protein` | `""` | Path to Protein file (optional, for gene names) |
| `--sample` | (required) | Path to sample file (CSV) |
| `--covariate` | (required) | Path to covariate file (CSV) |
| `--output` / `-o` | `.` | Output directory |
| `--backend` | `python` | `python` or `r` |
| `--seed` | None | Random seed for reproducibility |
| `--lod` | 0.01 | Limit of detection threshold |
| `--min-above` | 4 | Min channels above LOD |
| `--col-adjust` | 0.5 | SD percentile for normalization |
| `--outlier-cutoff` | 3.0 | Outlier residual threshold |
| `--scale-sn` | 1.0 | SNR scaling factor |
...
docker run --rm \
  ...
  mstrawler run \
    ...
    --covariate /data/covariate_file.csv \
    --output /output \
    --seed 42
...
docker run --rm \
  ...
  mstrawler run \
    ...
    --covariate /repo/data/tutorial_example/covariate_file.csv \
    --output /output \
    --seed 42
