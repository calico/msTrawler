...
#'  will be parameterized to include random deviations from the
#'  single fixed effect for the bridge.  If FALSE, then the bridge
#'  will be used to estimate fixed effects for each scan.  As a rule
#'  of thumb randScan should only be used on large datasets, i.e.
#'  > 10 batches.  On our system, randScan becomes essential to
#'  reduce computing time when the number of batches exceeds ~20.
...
#Step1.  Make sure the covariate file and sample file are consistent
  ...
  if(ncol(snMat) != ncol(iMat)){stop("Error: The number of SNR columns
  ...
  levelNames <- list()
    for(i in 1:n_cat){
      ...
      #Update table names
      tableNames <- c(tableNames, paste0("Factor_",
                                         factorNames[i], "_",
                                         levelNames[[i]], ".csv"))
    }#End level loop
  ...
  if(length(timeIndex) > 0){
    ...
    if(length(tCatIndex) > 1){stop("Only one time category is allowed")}
  ...
  tableList <- list()
    ...
    #Generate the sample estimate table
      ...
      tempTab <- data.frame(uGene, uProt, rep(NA, length(uGene)),
                            matrix(NA, nrow = length(uProt), ncol=resCols))
      colnames(tempTab) <- c("Gene", "Protein", "modelFit",
        paste0(rep(c("lower_", "est_", "upper_"), length(sampleNames)),
    ...
    for(i in 1:length(factorNames)){
      ...
      for(j in 1:levelN){
        ...
        tempTab <- data.frame(uGene, uProt, rep(NA, length(uGene)),
                              matrix(NA, nrow = length(uProt), ncol=resCols))
        colnames(tempTab) <- c("Gene", "Protein","modelFit",
          paste0(rep(c("Est_", "Pval_", "Qval_"), length(levelN-1)),
    ...
    if(timeLevelN == 1){
      ...
      for(l_ in 1:timeLevelN){
        tempTab <- data.frame(uGene, uProt, rep(NA, length(uGene)),
                              matrix(NA, nrow = length(uProt),
                                     ncol=nParams + nTests*2))
        colStr <- c("Gene", "Protein", "modelFit", timeVars,
                               paste0(rep(c("Pval_Time_", "Qval_Time_"), length(timeLevelN)),
...
#Now loop through references and fit the full model for each protein
  for(prot in 1:length(uProt)){
    ...
    tempRes <- try(adaptModel(notBridge, covType = "None",
               fixedForm, bridgeDat,  tParm,
               fullColumns = sampleNames, randScan, randID,
    ...
    for(i in 1:length(factorNames)){
      ...
      for(j in 1:levelN){
        ...
        tempRes <- try(adaptModel(protDat = notBridge,
                                  covType = "Factor", fixedForm = tempForm,
                                  bridgeDat,  tParm, lhtList,
                                  fullColumns, randScan, randID,
    ...
    if(n_cont > 0){
      ...
      tempRes <- try(adaptModel(protDat = notBridge,
                                ...
                                bridgeDat,  tParm, lhtList,
                                fullColumns = contNames, randScan, randID,
    ...
    if(tParm == "Time"){
      ...
      tempRes <- try(adaptModel(protDat = notBridge,
                                ...
                                bridgeDat,  tParm, lhtList,
                                fullColumns = timeVars, randScan, randID,
...
if(length(timeTables) > 0){
  for(z_ in 1:length(timeTables)){
    ...
    write.csv(tempTable, file = timeTableNames[z_],
              row.names=FALSE)
  }
 ...
 # if(bridgeMod == TRUE){
  ...
  for(s_ in 1:length(dfList)){
    ...
    save(readyDf, bridgeMod,coVars,tableNames,timeTableNames,tCatName,
         ...
         tParm, randScan, randID, tCatFactorIndex, tCatIndex, timeDiff,
...
#' @param rFile An rda datafile created by protPrep.  This is a
...
miniTrawl <- function(rdaSubset){
  ...
  for(z_ in 1:length(tableList)){
    tempTable <- tableList[[z_]]
    #We don't want to do an FDR adjustment on subsets!
    # qIndex <- grep("Qval", colnames(tempTable))
    # if(length(qIndex) > 0){
    #   for(q in 1:length(qIndex)){
    #     tempTable[ , qIndex[q]] <- p.adjust(tempTable[ , qIndex[q] - 1],
