...
findOutliers <- function(protMat, snMat, outlierCutoff = 3,
  ...
  mod <- lm(value ~ variable + 0, weights = I(scaleSN * minSN),
            data=melted)
  ...
  residMat <- reshape2::dcast(data.frame(cbind(melted, fullResid)),
                    Scan ~ variable, value.var = "fullResid")
...
adaptModel <- function(protDat, covType, fixedForm,
  ...
  if(covType == "None"){
    ...
    for(p_ in 1:nPlexes){
      ...
      if(bridgeMod == FALSE){
          tempMod <- try(lm(lIntensity ~ 0 + SampleID, weights = 1 / techVar,
                        data = subProt), silent = TRUE)
        ...
        allDat <- rbind(subProt, subBridge)  #Here's the mistake.  subProt contains bridge channels.
        ...
        if(length(levels(subProt$SampleID)) > 1){
  ...
  #We will figure out what to report from the model later
    if(is.null(bridgeDat)){
      ...
      if(nScans == 1 | (nBatches == 1 & randScan==TRUE)){
        #Directly create intercept when relevant factors has 1 level
        scanX <- matrix(1, nrow = nrow(allDat), ncol = 1)
      }else{
        if(randScan == TRUE){
          #Bridge intercept for each batch
          scanX <- model.matrix(~ 0 + Plex, data = allDat)
        }else{
      ...
      if(randScan == TRUE){#Fit the model with random scan effects
        if(randID == "SampleID"){
          ...
          lmerStr <- paste0("y_ ~ X_ + 0 + (1|Scan) + (0 + sampleI | subjectID) + (0 + sampleI | subjectID:sampleID)")
          lmerForm <- as.formula(lmerStr)
          lmerStr2 <- paste0("y_ ~ X_ + 0 + (1|Scan) + (0 + sampleI | sampleID)")
          lmerForm2 <- as.formula(lmerStr2)
        }
      }else{#Fit the model with fixed scan effects
        if(randID == "SampleID"){
      ...
      tempMod <- try(lme4::lmer(lmerForm,
                                weights = 1 / w_,
                                control = lme4::lmerControl(check.nobs.vs.nlev = "stop",
      ...
      if(covType == "Factor"){
        ...
        if(modelType == "lmer" | modelType == "lmer2"){
          ...
          for(j in 1:length(fullColumns)){
            ...
            if(length(badMatch) > 0){next}


            targetRow <- grep(paste0(fullColumns[j], "$"), names(estPars))
            if(length(targetRow) == 0){next}#If we didnt' get an estimate
            #then move on
        ...
        if(length(lhtList) > 0){
          ...
          for(test_ in 1:length(lhtList)){
            ...
            if(is.na(lhtList[[test_]][1])){
              ...
              #Do the test
              if(modelType == "lm"){
                tempTest <- try(car::lht(tempMod, hypothesis.matrix = testMat, singular.ok=T,
                                         test="F")[2, "Pr(>F)"], silent = T)
              }else{
...
#Now extract continuous covariate
  ...
  #a continuous or time covariate.  Check for time tests.
  if(length(lhtList) > 0){
    #First we have to do the time related hypothesis tests
    n_timeParams <- length(lhtList[[1]])
    timeRes <- rep(NA, n_timeParams + length(lhtList) * 2)

    #Populate point estimates of reference parameters
    timePars <- substring(lhtList[[1]], 1, regexpr("=", lhtList[[1]]) - 2)
    for(par_ in 1:n_timeParams){
      targetRow <- grep(timePars[par_], rownames(tTable))
        timeRes[par_] <- tTable[targetRow, 1]

    }
    for(test_ in 1:length(lhtList)){
      tempTest <- try(car::lht(tempMod, lhtList[[test_]])[2, 3])
